...
import { fetchProduct, type Product } from "@/lib/hygraph"
...
interface ProductWithSizeGuide extends Product {
  sizeGuide?: {
...
export default function ProductDetail() {
  ...
  useEffect(() => {
    async function loadProduct() {
      try {
        ...
        //   console.log("Default color selected:", productData.colour[0]) // Debug log
        // }
        if (productData.size && productData.size.length > 0) {
          setSelectedSize(productData.size.find((s) => s.inStock) || productData.size[0])
        }
        if (productData.quantity && productData.quantity.length > 0) {
          setSelectedPiece(productData.quantity.find((q) => q.inStock) || productData.quantity[0])
        }
      } catch (error) {
  ...
  // Parse highlights string into array if it's a string
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
            ...
            <div className="order-1 lg:order-none">
              <div className="mt-8 space-y-4">
                ...
                <p className="text-base sm:text-lg text-gray-700 font-bold leading-relaxed">{product.description}</p>
...
{product.colour &&
  Array.isArray(product.colour.colour) &&
  Array.isArray(product.colour.colourName) &&
  product.colour.colour.length === product.colour.colourName.length && (
    <div className="mt-10">
      ...
      <fieldset aria-label="Choose a color" className="mt-4">
        ...
        >
          {product.colour.colour.map((color, index) => {
            const name = product.colour.colourName[index]

            return (
              <RadioGroup.Option
                key={index}
                value={{ colour: color, colourName: name }}
              >
                {({ checked }) => (
                  <div
                    className={classNames(
                      "group relative flex items-center justify-center rounded-md px-4 py-3 text-sm font-medium uppercase",
                      "cursor-pointer bg-white text-gray-900 transition-colors duration-150",
                      "hover:border-blue-500 border-2",
                      checked ? "border-2 border-blue-500" : "border border-gray-200"
                    )}
                  >
                    <span className="flex flex-col items-center gap-1">
                      <span
                        className={classNames(
                          "inline-block size-6 rounded-full border border-gray-300",
              ...
              {product.quantity?.length > 0 && (
                <div className="mt-10">
                  ...
                  <fieldset aria-label="Choose a quantity" className="mt-4">
                    <RadioGroup
                      value={selectedPiece}
                    ...
                    >
                      {product.quantity.map((item) => {
                        const status = item.inStock ? "Active" : "Inactive"
              ...
              <a
                href={`https://wa.me/919600668214?text=${encodeURIComponent(
                  `Hello Sir, I'm interested in the following product:

Product Name: ${product.name}
GSM Quality: ${product.gsm}
Color: ${selectedColor?.colourName || selectedColor?.colour?.hex || "Not selected"}
Size: ${selectedSize?.piece || "Not selected"}
Pieces: ${selectedPiece?.piece || "Not selected"}
Price: ₹${product.price}
...
Please let me know the next steps.`
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-6 w-full inline-flex items-center justify-center gap-2 rounded-md border border-green-600 bg-green-600 px-8 py-3 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 transition-colors duration-200"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M17.472 14.382c-..." /> {/* your SVG icon path */}
                </svg>
                WhatsApp for Order
              </a>
            ...
            <img
              src={product.image.url || "/placeholder.svg?height=600&width=600"}
              alt={product.name}
            ...
            <div className="order-3 lg:col-span-2">
              {highlightsArray.length > 0 && (
                <>
                  ...
                  <ul className="mt-2 list-disc pl-5 text-md text-gray-800 space-y-1">
                    {highlightsArray.map((item, i) => (
